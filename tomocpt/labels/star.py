...
class Star:
    ...
    def _initialize_star_properties(self):
        ...
        if "optics" in self.star_data:
            ...
            self.pixel_size = (
                pd.Series([1])
                if not self.star_data["rlnPixelSize"].size
                else self.star_data["rlnPixelSize"]
            )
    ...
    def get_particle_subset(self, class_id: str) -> Tuple[pd.DataFrame, str]:
        ...
        if "rlnClassNumber" not in self.particle_data.columns:
            logging.warn(
                "rlnClassNumber column not found in particle data. Setting class_id to 'all'."
            )
        ...
        if "all" in class_id:
            ...
            subset_data = self.particle_data[
                self.particle_data["rlnClassNumber"].isin(class_id_list)
            ]
    ...
    def get_shifted_scaled_coordinates(
        self, particle_data: pd.DataFrame, tomogram_pixel_size: Union[pd.Series, float]
    ) -> Dict[str, np.ndarray]:
        """
        ...
        if missing_shifts:
            logging.warn(
                f"Missing shift headings in particle data: {missing_shifts}. Setting shifts to 0."
            )
        ...
        tomo_pixel_size = (
            tomogram_pixel_size.iloc[0]
            if isinstance(tomogram_pixel_size, pd.Series)
            else tomogram_pixel_size
        )
        ...
        return {
            str(name): group[xyz_headings].astype(int).values
