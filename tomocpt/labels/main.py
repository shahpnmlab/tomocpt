...
@cli.command()
def prepare_picking_imod(mod_file_path: Path = Option(..., "--input-mod-file", "-i", help="Path to mod files"),
...
@cli.command()
def prepare_picking_star(star_file: Path = Option(..., "--input-star-file", "-i", help="Path to STAR file"),
                         tomo_path: Path = Option(..., "--tomo-path", "-t", help="Path to tomograms"),
                         output_dir: Path = Option(..., "--output", "-o", help="Output directory"),
                         class_id: str = Option("all", "--class-id", "-c", help="Class ID(s) to process"),
                         particle_diameter_angst: float = Option(..., "--length", "-l",
                                                                 help="Particle longest axis length (Å)")
                         ) -> None:
    particle_radius_angst = particle_diameter_angst / 2.0
