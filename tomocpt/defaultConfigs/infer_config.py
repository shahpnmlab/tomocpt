...
class InferConfig:
    ...
    masks_dir: Annotated[Optional[Path], typer.Option(help="Directory containing masks for targeted picking. Masks should have the same name as tomogram name.")] = None
    length: Annotated[float, typer.Option(help="Particle diameter in Angstroms if spherical, otherwise the longest axis")] = MISSING
    distance_threshold: Annotated[Optional[float], typer.Option(help="Filter out particle closer than this distance in Angstroms")] = None
    predictions_coord_filename: Annotated[str, typer.Option(help="Name of the output star file.")] = "tomopicker_coords.star"
    predictions_coord_format:  Annotated[OutputFormat, typer.Option(help="Format for the output star file")] = OutputFormat.relion31
    ...
    n_cpus_per_gpu: Annotated[int, typer.Option(help="Number of CPU workers per GPU to pre-process data")] = 1
