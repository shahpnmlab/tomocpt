    ...
    for tomo_fname in tqdm(tomo_fnames, desc=f"Inferring on {device}", position=gpu_id or 0, leave=False):
        try:
            ...
            if infer_config.save_predicted_coords:
                ...
                int_coords_gpu = peak_local_max_torch(image=pred_mask_gpu, min_distance=int(min_dist_px),
                ...
                if int_coords_gpu.shape[0] > 0:
                    ...
                    centroids_with_scores = torch.hstack(
                        [refined_coords_gpu, refined_scores_gpu.unsqueeze(-1)]).cpu().numpy()
                    results_aggregator.append((tomo_fname.stem, centroids_with_scores.tolist(), original_voxel_size))
        except Exception as e:
