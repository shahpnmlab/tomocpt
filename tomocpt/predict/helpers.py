...
def _infer_one_tomo(
    ...
    """
    Enhanced inference for a single tomogram with proper device management
    """
    ...
    if not Path(output_fname).exists():
        # Preprocess with proper device specification
        ...
        # Add batch dimension and create TorchIO subject
        vol = vol.unsqueeze(0)
        ...
        with torch.inference_mode():
            # Use tqdm instead of print statements
            for patches_batch in tqdm(
                    patch_loader,
                    desc=f"Processing {Path(tomoFname).stem}",
                    position=0,
                    leave=True
            ):
                ...
                with torch.amp.autocast(device.type):
                    outputs = model.predict_step(input_tensor, 0)
        ...
        # Process full volume prediction
        ...
        if maskFname and Path(maskFname).exists():
            try:
                ...
                if mask.shape != output_tensor.shape:
                    raise ValueError(
                        f"Mask shape {mask.shape} does not match volume shape {output_tensor.shape}"
                    )
    ...
    if extract_coords:
        ...
        if len(coords_array) == 0:
            logger.info(f"No particles found in {Path(tomoFname).stem}")
        else:
