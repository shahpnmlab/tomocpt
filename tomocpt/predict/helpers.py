...
def unpad(inp_tensor: torch.Tensor, padding_values: Tuple[int, ...]) -> torch.Tensor:
    ...
    z_pad_begin, z_pad_end, y_pad_begin, y_pad_end, x_pad_begin, x_pad_end = (
        padding_values
    )
...
def write_segmentation_mask(
    tensor_mask, outputname: str, angpix: float = 1.0, overwrite=True
):
    ...
    mrcfile.write(
        output_path, data=m.astype(np.float16), voxel_size=angpix, overwrite=overwrite
    )
...
def write_imod_model(
    ...
    if not predicted_centroids_with_scores:
        logger.warning(
            f"No coordinates found for {tomo_name}, skipping .mod file creation."
        )
        return
...
def write_relion_star_file(
    ...
    for tomoName, centroid in zip(tomo_names, predicted_centroids_with_scores):
        ...
        particles_data["rlnOpticsGroup"].append(1)
...
def write_warp_star_file(
    ...
    output_path = Path(output_dir) / output_filename
    starfile.write(
        {"particles": df_particles}, output_path, float_format="%.2f", overwrite=True
    )
    logger.info(f"Predicted coordinates are stored in: {output_path}")
    return output_path
...
def peak_local_max_torch(
    ...
    for i in range(candidates.shape[0]):
        ...
        mask_z_min, mask_z_max = dist - (coord[0] - z_min), dist + (
            z_max - coord[0] - 1
        )
        mask_y_min, mask_y_max = dist - (coord[1] - y_min), dist + (
            y_max - coord[1] - 1
        )
        mask_x_min, mask_x_max = dist - (coord[2] - x_min), dist + (
            x_max - coord[2] - 1
        )
...
def _prune_refined_peaks_gpu(
    coords: torch.Tensor, scores: torch.Tensor, min_distance: float
    ...
    for i in range(sorted_coords.shape[0]):
        ...
        distances_sq = torch.sum(
            (sorted_coords[i + 1 :] - sorted_coords[i]) ** 2, dim=1
        )
...
def _refine_peaks_subpixel_gpu(
    ...
    if peak_coords_gpu.shape[0] == 0:
        return torch.empty_like(peak_coords_gpu, dtype=torch.float32), torch.empty(
            0, device=pred_mask_gpu.device, dtype=torch.float32
        )
    ...
    patch_coords_offsets = torch.stack(
        torch.meshgrid(
            torch.arange(
                -patch_radius, patch_radius + 1, device=device, dtype=torch.long
            ),
            torch.arange(
                -patch_radius, patch_radius + 1, device=device, dtype=torch.long
            ),
    ...
    patches_coords[:, 0, ...] = torch.clamp(
        patches_coords[:, 0, ...], 0, shape_tensor[0] - 1
    )
    patches_coords[:, 1, ...] = torch.clamp(
        patches_coords[:, 1, ...], 0, shape_tensor[1] - 1
    )
    patches_coords[:, 2, ...] = torch.clamp(
        patches_coords[:, 2, ...], 0, shape_tensor[2] - 1
    )
...
def process_extracted_coordinates(
    ...
    if output_format == OutputFormat.relion_31:
        ...
        df = pd.DataFrame(
            {"tomo_name": tomo_names, "coords": predicted_centroids_with_scores}
        )
        for name, group in df.groupby("tomo_name"):
            ...
            write_imod_model(
                ...
                tomo_name=name,
                predicted_centroids_with_scores=tomo_coords,
...
def _get_prediction_mask_with_tta(
    ...
    for i, transform in enumerate(tta_transforms):
        ...
        patch_loader = DataLoader(
            grid_sampler, batch_size=infer_config.predictions_batch_size, num_workers=0
        )
        ...
        with torch.inference_mode():
            for batch in patch_loader:
                ...
                with torch.amp.autocast(
                    device_type=device.type, enabled=infer_config.use_cuda
                ):
...
def _predict_single_tomogram(
    ...
    if infer_config.masks_dir:
        ...
        if mask_path.exists():
            ...
            logger.warning(
                f"Mask file not found for {tomo_fname.name} at {mask_path}, proceeding without it."
            )
...
def infer_tomos(
    ...
    warnings.filterwarnings(
        "ignore", message=".*`img_size` has been deprecated.*", category=FutureWarning
    )
    warnings.filterwarnings(
        "ignore",
    ...
    from tomocpt.networks.inference import InferenceModel
    ...
    try:
        # InferenceModel will automatically filter teacher weights from checkpoints
        model = InferenceModel.load_from_checkpoint(
    ...
    try:
        ...
        logger.warning(
            "Could not find 'desired_particle_pixel_size' in model. Falling back to global config."
        )
    ...
    with tqdm(
        ...
        for tomo_fname in tomo_fnames:
            try:
                ...
                result = _predict_single_tomogram(
                    model,
                    tomo_fname,
                    infer_config,
                    target_px_from_model,
                    device,
                    gpu_id,
                    pbar,
                )
