...
def process_single_tomo_task(task: Dict[str, Any]) -> tuple:
    ...
    """
    try:
        tomo_info = task["tomo_info"]
        outpath = task["outpath"]
        cache_dir = task["cache_dir"]
        require_labels = task["require_labels"]
        desired_particle_pixel_size = task["desired_particle_pixel_size"]
        ...
        transform_pipeline = PreprocessTomogramd(
            tomo_key="tomo",
            label_key="label",
            particle_diameter_key="diameter",
            device=device,
            target_particle_px=desired_particle_pixel_size
        )
        ...
        try:
            ...
            if require_labels and not (tomo_path_str == tomo_info["label_path"]):
                ...
                if f0 > 0:
                    drop_probability = torch.clip(1 - mainConfig.prepData.ALPHA_FOR_DROPPING_EMPTY_CUBES * (f1 / f0),
                                                  min=0.0, max=1.0).item()
        ...
        with ThreadPoolExecutor(max_workers=n_threads) as executor:
            ...
            for i, patch in enumerate(dataloader):
                ...
                vol_path = str(
                    vol_dir / f"{constants.VOLUMES_DIR_NAME_PREFIX}_{i}_{coords[0]}_{coords[1]}_{coords[2]}.nii.gz")
        ...
        logging.error(
            f"Dask worker processing {task.get('tomo_info', {}).get('tomogram_path', 'unknown file')} FAILED!")
